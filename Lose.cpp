...
bool Lose::Init()
{
	...
	title = D3D::LoadTexture("sprites/titles/missionfailed.png");
	button[0] = D3D::LoadTexture("sprites/buttons/playagain.png");
	button[1] = D3D::LoadTexture("sprites/buttons/mainmenu.png");
	buttonOver[0] = D3D::LoadTexture("sprites/buttons/over/playagain.png");
	buttonOver[1] = D3D::LoadTexture("sprites/buttons/over/mainmenu.png");
...
bool Lose::Render(float deltaTime)
{
	...
	for (int i = 0; i < 2; i++) {
		if (i != currentButton) {
			sprite->Draw(button[i], NULL, &buttonCenter, &D3DXVECTOR3(450, (float)300 + (100 * i), 0), D3D::WHITE);
		}
		else {
			sprite->Draw(buttonOver[i], NULL, &buttonCenter, &D3DXVECTOR3(450, (float)300 + (100 * i), 0), D3D::WHITE);
		}
	}
	sprite->End();
...
void Lose::Exit(GameState *nextState)
{
